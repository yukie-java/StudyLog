...
- 表示中の科目種別（adult / child）で一覧・検索・週 / 月 / 累計を自動フィルタ
- 表示切替に合わせて登録フォームの科目種別も自動追従

### 可視化・進捗機能
- 直近7日の学習時間を棒グラフで表示
- 科目別学習時間を円グラフで表示
- child：継続日数（streak）と累計学習時間に応じたジム進捗（全8段階）
- adult：今週の学習時間を先週の自分と比較する「自分対決」機能
- child：継続日数（streak）と累計学習時間に応じたジム進捗（全8段階）
  - ジムごとにボス画像を表示
  - レベルに応じてCLEAR / LOCKEDを切り替え
  - JSPで動的にジム一覧を生成
